...
/**
 * <p>
 * The class Rook_Replace implements an upper tester
 * </p>
 *
 * @author    Jörg Dippel
 * @version   June 2020
 *
 */
@DisplayName("the public method Rook replace( ) for class Rook is tested")
public class Rook_Replace {  
    
    @ParameterizedTest( name = "given a standard board a rook from location {0} moved to location {1} the movings must match {2}" )
    ...
    public void testWithMultiArgMethodSource_ForMovings( String origin, String target, List<String> expectedResultList ) {
        ...
        Rook piece = Rook.create( origin ).replace( target );
    ...
    public static Stream<Arguments> stringAndStringAndListProvider() {
        return Stream.of(
            ...
            Arguments.of( "a2", "a1", Arrays.asList( "a1 a2 a3 a4 a5 a6 a7 a8", "a1 b1 c1 d1 e1 f1 g1 h1" ) ),
            Arguments.of( "e7", "e4", Arrays.asList( "e4 e5 e6 e7 e8", "e4 f4 g4 h4", "e4 e3 e2 e1", "e4 d4 c4 b4 a4" ) ),
            Arguments.of( "b4", "h4", Arrays.asList( "h4 h5 h6 h7 h8", "h4 h3 h2 h1", "h4 g4 f4 e4 d4 c4 b4 a4" ) )
