...
/**
 * <p>
 * The class Queen_Replace implements an upper tester
 * </p>
 *
 * @author    Jörg Dippel
 * @version   June 2020
 *
 */
@DisplayName("the public method Queen replace( ) for class Queen is tested")
public class Queen_Replace {     
    
    @ParameterizedTest( name = "given a standard board a queen from location {0} moved to location {1} the movings must match {2}" )
    ...
    public void testWithMultiArgMethodSource_ForMovings( String origin, String target, List<String> expectedResultList ) {
        ...
        Queen piece = Queen.create( origin ).replace( target );
    ...
    @ParameterizedTest( name = "given a standard board a queen from location {0} moved to location {1} the capturings must match {2}" )
    @MethodSource("stringAndStringAndListProvider")
    ...
    public static Stream<Arguments> stringAndStringAndListProvider() {
        return Stream.of(
            ...
            Arguments.of( "a6", "a1", Arrays.asList( "a1 a2 a3 a4 a5 a6 a7 a8", "a1 b1 c1 d1 e1 f1 g1 h1", "a1 b2 c3 d4 e5 f6 g7 h8" ) ),
            Arguments.of( "b7", "e4", Arrays.asList( "e4 d3 c2 b1", "e4 d4 c4 b4 a4", "e4 d5 c6 b7 a8", "e4 e3 e2 e1", "e4 e5 e6 e7 e8", "e4 f3 g2 h1", "e4 f4 g4 h4", "e4 f5 g6 h7" ) ),
            Arguments.of( "h8", "h4", Arrays.asList( "h4 g3 f2 e1", "h4 g4 f4 e4 d4 c4 b4 a4", "h4 g5 f6 e7 d8", "h4 h3 h2 h1", "h4 h5 h6 h7 h8" ) )
